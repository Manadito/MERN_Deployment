...
const CompletedTasksList = (props) => {
  ...
  const { tasksList, removeTask } = props;

  return (
    <div className="col bg-custom-white d-flex justify-content-center align-items-start custom-styling-ctl">
      <ul className="list-unstyled custom-styling-ul-items">
        {tasksList && // Creating cards and avoiding data errors with && in the process
          _.orderBy(
            _.filter(tasksList, (item) => item.status === "completed"), // Filtering tasksList in order to keep those with an updated status of completed (patched)
            ["dueDate"],
            ["asc"]
          ).map((item, idx) => (
            <li
            ...
            >
              ...
              <div className="custom-styling-btn-container-width">
                {/*. Button will execute the function, upon click, to delete tasks from app and db */}
                <button
                  className="custom-styling-list-btns custom-btn-color-completed"
